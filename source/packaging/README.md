...
0.3.7 使用 Python 3.11、Tk、Win32 和 PyInstaller。入口为 `loot_app.py`；设置面板与鼠标穿透标记层分开，读取工作在独立后台进程中进行。
...
产物为 `releases/0.3.7/RuinsLootHelper-0.3.7-Windows-x64.zip`，可运行目录为 `release-staging-0.3.7/破晓装备助手/`。
